...
// Copied from Satchel.BetterMenus
public class AttackWeightSlider : Element //should be renamed to slider probs and probably needs to be remade slider needs to be added to nav graph
{
    ...
    /// <summary>
    /// Creates a new AttackWeightSlider instance.
    /// </summary>
    ...
    public AttackWeightSlider(string name, Action<float> storeValue, Func<float> loadValue, string Id = "__UseName") : base(Id, name)
    {
        Name = name;
    ...
    /// <param name="Id">the id of the element that can be used to search for it</param>
    public AttackWeightSlider(string name, Action<float> storeValue, Func<float> loadValue, float minValue, float maxValue, bool wholeNumbers = false, string Id = "__UseName") : base(Id, name)
    {
        Name = name;
        StoreValue = storeValue;
        LoadValue = loadValue;
    ...
    public static GameObject prefab
    {
        get
        {
            if (_prefab == null)
            {
                ...
                GameObject.DestroyImmediate(newPrefab.GetComponent<Slider>());
                ...
                GameObject.DestroyImmediate(newPrefab.GetComponent<MenuAudioSlider>());
                GameObject.DestroyImmediate(newPrefab.transform.Find("Label")?.GetComponent<AutoLocalizeTextUI>());
    ...
    public Text label
    {
        get
        {
            if (_label == null)
            {
                _label = currentSlider.transform.Find("Label")?.GetComponent<Text>();
            }
            return _label;
        }
    }
    public Text valueLabel
    {
        get
        {
            if(_valueLabel == null)
            {
                _valueLabel = currentSlider.transform.Find("MusicValue")?.GetComponent<Text>();
            }
            return _valueLabel;
        }   
    }
    ...
    {
        valueLabel.text = wholeNumbers ? $"{value}" : $"{value:0.00}";
    }
}